					...
					else
					{
						...
						do
						{
							...
							{												
								...
								if ((alumnos[ocupados].fechaNacimiento.dia <=31) &&
										(alumnos[ocupados].fechaNacimiento.dia >0) &&
										(alumnos[ocupados].fechaNacimiento.mes >0) &&
										(alumnos[ocupados].fechaNacimiento.mes <=12) &&
										(alumnos[ocupados].fechaNacimiento.anyo >= 1900) &&
										(alumnos[ocupados].fechaNacimiento.anyo <= 2010))
								{
									fecha_OK = true;
								}
								else
						...
						{				
							for (int i = 0; i < PRACTICAS; i++)
							{
								do
								{
									Console.Write("Introduce nota de la práctica {0}: ", i+1);
									alumnos[ocupados].nota.practica[i] =
										Convert.ToSingle(Console.ReadLine());
								}
								while (!(alumnos[ocupados].nota.practica[i] >= 0 &&
									alumnos[ocupados].nota.practica[i] <= 10));
							}
							...
							for (int i = 0; i < EXAMENES; i++)
							{
								do
								{
									Console.Write("Introduce nota de examen {0}: ", i+1);
									alumnos[ocupados].nota.examen[i] = Convert.ToInt32(Console.ReadLine());
								}
								while (!(alumnos[ocupados].nota.examen[i] >= 0 &&
									alumnos[ocupados].nota.examen[i] <= 10));
							}

							// Calculo de la nota final
							notaAUX = 0.0f;
							notaAUX = (0.3f * ((alumnos[ocupados].nota.practica[0] +
								alumnos[ocupados].nota.practica[1]) * 0.5f) + 0.7f *
								alumnos[ocupados].nota.examen[0]);
								alumnos[ocupados].nota.final = 0.2f * notaAUX;

							notaAUX = (0.3f * ((alumnos[ocupados].nota.practica[2] +
								alumnos[ocupados].nota.practica[3]) * 0.5f) + 0.7f *
								alumnos[ocupados].nota.examen[1]);
								alumnos[ocupados].nota.final += 0.3f * notaAUX;

							notaAUX = (0.3f * ((alumnos[ocupados].nota.practica[4] +
								alumnos[ocupados].nota.practica[5]) * 0.5f) + 0.7f *
								alumnos[ocupados].nota.examen[2]);
				...
				case "2":
					...
					else
					{
						for (int i=0; i<ocupados; i++)		// Muestra toda la lista de contactos
						{
							...
							Console.WriteLine("Fecha Nacimiento: {0}/{1}/{2}",
								alumnos[i].fechaNacimiento.dia,	alumnos[i].fechaNacimiento.mes,
					...
					else
					{
						...
						for (int i=0; i < ocupados; i++)
						{
							if (alumnos[i].ciudad.ToUpper().Contains(buscarCiudad.ToUpper()))
							{
								...
								Console.WriteLine("Ciudad: {0}", alumnos[i].ciudad);
								Console.WriteLine("Fecha Nacimiento: {0}/{1}/{2}", alumnos[i].fechaNacimiento.dia,
									alumnos[i].fechaNacimiento.mes, alumnos[i].fechaNacimiento.anyo);
								Console.WriteLine("Nota final: {0}", alumnos[i].nota.final);
							}
						}
						if (encontrado == false)
